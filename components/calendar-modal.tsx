    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 relative">
          ...
          <div className="grid grid-cols-7 gap-1 text-center">
            ...
            {Array.from({ length: daysInMonth }, (_, i) => {
              ...
              return (
                <div
                  key={day}
                  className={`relative p-2 h-12 flex items-center justify-center text-sm transition-all duration-200 hover:bg-muted/50 rounded-lg cursor-pointer ${
                  ...
                  {dayTasks.length > 0 && (
                    <div className="absolute -bottom-1 -right-1 flex space-x-0.5">
                      {dayTasks.slice(0, 3).map((task, index) => (
                        <div
                          key={task.id}
                          className={`w-2 h-2 ${getTaskColor(task.type)} rounded-full border border-white animate-pulse`}
                        ></div>
                      ))}
                      {dayTasks.length > 3 && (
                        <div className="w-2 h-2 bg-gray-500 rounded-full border border-white text-xs flex items-center justify-center">
                          +
                        </div>
                      )}
                    </div>
          ...
          {showTaskForm && (
            <div className="absolute inset-0 bg-white dark:bg-gray-900 rounded-lg border shadow-lg z-50 p-4">
              <div className="flex items-center justify-between mb-4">
                <h4 className="text-lg font-semibold">
                  {selectedDate && `Tareas para el ${selectedDate} de ${monthNames[currentMonth]}`}
                </h4>
                <Button variant="ghost" size="sm" onClick={closeTaskForm}>
                  <X className="w-4 h-4" />
                </Button>
              </div>

              {/* Tareas existentes para la fecha */}
                  ...
                  {getTasksForDay(selectedDate).map((task) => (
                    <div key={task.id} className="flex items-center justify-between p-2 bg-muted/30 rounded-lg">
                      <div className="flex items-center space-x-2">
                        <div className={`w-3 h-3 ${getTaskColor(task.type)} rounded-full`}></div>
                        <div>
          ...
          {/* Legend - solo visible cuando no hay formulario */}
          {!showTaskForm && (
              ...
              <div className="space-y-2">
                {tasks.slice(0, 3).map((task, index) => (
                  <div key={index} className="flex items-center justify-between p-2 bg-muted/30 rounded-lg">
                    <div className="flex items-center space-x-2">
                      <div className={`w-3 h-3 ${getTaskColor(task.type)} rounded-full animate-pulse`}></div>
                      <span className="text-sm">{getTaskLabel(task.type)}</span>
                    </div>
